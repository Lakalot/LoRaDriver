...
LoRaError LoRaTransceiver::begin(const LoRaConfig& cfg) noexcept {
    ...
    driver_.set_event_callback([this](RadioEvent ev, int param) { on_driver_event(ev, param); });
...
void LoRaTransceiver::on_receive(PacketCallback cb) noexcept {
    packet_cb_ = std::move(cb);
}
void LoRaTransceiver::on_event(EventCallback cb) noexcept {
    event_cb_ = std::move(cb);
}
void LoRaTransceiver::on_tx_done(TxDoneCallback cb) noexcept {
    tx_done_cb_ = std::move(cb);
}
void LoRaTransceiver::on_header(HeaderCallback cb) noexcept {
