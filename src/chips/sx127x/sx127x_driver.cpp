...
std::uint8_t ocp_trim(std::uint8_t ma) noexcept {
    ...
    if (ma <= 120u)
        return static_cast<std::uint8_t>((ma - 45u) / 5u);
    if (ma <= 240u)
        return static_cast<std::uint8_t>((ma + 30u) / 10u);
...
LoRaError SX127xDriver::set_op_mode(std::uint8_t mode) noexcept {
    ...
    const bool needs_verify = (mode == opmode::kLoRaTx || mode == opmode::kLoRaRxCont ||
                               mode == opmode::kLoRaRxSingle || mode == opmode::kLoRaCad);
    if (needs_verify) {
        std::uint8_t readback = 0;
        e = spi_.read_register(reg::kOpMode, readback);
        if (e != LoRaError::OK)
            return e;
        if ((readback & opmode::kLoRaModeBit) == 0u)
            return LoRaError::SpiVerifyMismatch;
    }
...
LoRaError SX127xDriver::apply_frequency(std::uint32_t hz) noexcept {
    ...
    LoRaError e;
    if ((e = spi_.write_register(reg::kFrMsb, static_cast<std::uint8_t>((frf >> 16) & 0xFF))) !=
        LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kFrMid, static_cast<std::uint8_t>((frf >> 8) & 0xFF))) !=
        LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kFrLsb, static_cast<std::uint8_t>(frf & 0xFF))) !=
        LoRaError::OK)
        return e;
...
LoRaError SX127xDriver::apply_modem_config(const LoRaConfig& cfg) noexcept {
    ...
    const std::uint8_t mc1 = static_cast<std::uint8_t>((bw_code(cfg.bandwidth_hz) << 4) |
                                                       (cr_code(cfg.coding_rate) << 1) |
                                                       (cfg.implicit_header ? 0x01u : 0x00u));
    LoRaError e;
    if ((e = spi_.write_register(reg::kModemConfig1, mc1)) != LoRaError::OK)
        return e;

    // ModemConfig2: SF[7:4] | TxContinuous[3]=0 | CRC[2] | SymbTimeoutMsb[1:0]
    const std::uint16_t symb_to =
        (cfg.symbol_timeout > 0x3FFu) ? std::uint16_t{0x3FFu} : cfg.symbol_timeout;
    const std::uint8_t mc2 = static_cast<std::uint8_t>(
        (cfg.spreading_factor << 4) | (cfg.crc_enabled ? 0x04u : 0x00u) | ((symb_to >> 8) & 0x03u));
    if ((e = spi_.write_register(reg::kModemConfig2, mc2)) != LoRaError::OK)
        return e;

    // SymbTimeoutLsb
    if ((e = spi_.write_register(reg::kSymbTimeoutLsb,
                                 static_cast<std::uint8_t>(symb_to & 0xFFu))) != LoRaError::OK)
        return e;

    // ModemConfig3: LDRO[3] | AgcAuto[2]
    const bool ldro = cfg.ldro_auto && cfg.ldro_required();
    const std::uint8_t mc3 =
        static_cast<std::uint8_t>((ldro ? 0x08u : 0x00u) | (cfg.agc_auto ? 0x04u : 0x00u));
    if ((e = spi_.write_register(reg::kModemConfig3, mc3)) != LoRaError::OK)
        return e;
    ...
    if (cfg.spreading_factor == 6u) {
        if ((e = spi_.write_register(reg::kDetectionOptimize, 0x05)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kDetectionThreshold, 0x0C)) != LoRaError::OK)
            return e;
    } else {
        if ((e = spi_.write_register(reg::kDetectionOptimize, 0x03)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kDetectionThreshold, 0x0A)) != LoRaError::OK)
            return e;
    }
    return LoRaError::OK;
}
...
LoRaError SX127xDriver::apply_tx_power(std::int8_t dbm, PaOutput out) noexcept {
    ...
    if (out == PaOutput::Rfo) {
        ...
        const std::uint8_t v =
            static_cast<std::uint8_t>(0x70u | (static_cast<std::uint8_t>(dbm) & 0x0Fu));
        if ((e = spi_.write_register(reg::kPaConfig, v)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kPaDac, 0x84)) != LoRaError::OK)
            return e;
    } else {
        ...
        const std::uint8_t out_pow =
            static_cast<std::uint8_t>(high_power ? (clamped - 5) : (clamped - 2)) & 0x0Fu;
        const std::uint8_t v = static_cast<std::uint8_t>(0x80u | 0x70u | out_pow);
        if ((e = spi_.write_register(reg::kPaConfig, v)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kPaDac, high_power ? 0x87u : 0x84u)) != LoRaError::OK)
            return e;
    }
    return LoRaError::OK;
}
...
LoRaError SX127xDriver::apply_errata(std::uint32_t bw_hz, std::uint32_t freq_hz) noexcept {
    ...
    if (bw_hz == 500'000u && freq_hz >= 525'000'000u) {
        if ((e = spi_.write_register(reg::kHighBwOptimize1, 0x02)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kHighBwOptimize2, 0x64)) != LoRaError::OK)
            return e;
    } else {
        if ((e = spi_.write_register(reg::kHighBwOptimize1, 0x03)) != LoRaError::OK)
            return e;
    }
    ...
    if (bw_hz < 500'000u) {
        std::uint8_t det = 0;
        if ((e = spi_.read_register(reg::kDetectionOptimize, det)) != LoRaError::OK)
            return e;
        det &= ~0x80u;
        ...
        if ((e = spi_.write_register(reg::kDetectionOptimize, det)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kIfFreq1, 0x00)) != LoRaError::OK)
            return e;
        if ((e = spi_.write_register(reg::kIfFreq2, 0x00)) != LoRaError::OK)
            return e;
    }
...
LoRaError SX127xDriver::apply_init_sequence(const LoRaConfig& cfg) noexcept {
    ...
    // Detect chip
    if ((e = spi_.read_register(reg::kVersion, chip_version_)) != LoRaError::OK)
        return e;
    if (chip_version_ != kVersionExpected)
        return LoRaError::UnsupportedChip;

    // TCXO config: if enabled, set TcxoInputOn (bit 4). Datasheet §6.6.
    {
        std::uint8_t tcxo = 0;
        if ((e = spi_.read_register(reg::kTcxo, tcxo)) != LoRaError::OK)
            return e;
        if (cfg.tcxo_enabled)
            tcxo |= 0x10u;
        else
            tcxo &= ~0x10u;
        if ((e = spi_.write_register(reg::kTcxo, tcxo)) != LoRaError::OK)
            return e;
    }
    ...
    std::uint8_t op = 0;
    if ((e = spi_.read_register(reg::kOpMode, op)) != LoRaError::OK)
        return e;
    if ((op & opmode::kLoRaModeBit) == 0u)
        return LoRaError::SpiVerifyMismatch;

    // Frequency
    if ((e = apply_frequency(cfg.frequency_hz)) != LoRaError::OK)
        return e;

    // PA + OCP
    if ((e = apply_tx_power(cfg.tx_power_dbm, cfg.pa_output)) != LoRaError::OK)
        return e;
    if ((e = apply_ocp(cfg.ocp_ma)) != LoRaError::OK)
        return e;

    // Modem
    if ((e = apply_modem_config(cfg)) != LoRaError::OK)
        return e;

    // Sync word
    if ((e = spi_.write_register(
             reg::kSyncWord, static_cast<std::uint8_t>(cfg.sync_word & 0xFFu))) != LoRaError::OK)
        return e;

    // Preamble length
    if ((e = spi_.write_register(reg::kPreambleMsb, static_cast<std::uint8_t>(cfg.preamble_length >>
                                                                              8))) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kPreambleLsb,
                                 static_cast<std::uint8_t>(cfg.preamble_length & 0xFFu))) !=
        LoRaError::OK)
        return e;

    // LNA boost
    const std::uint8_t lna = static_cast<std::uint8_t>((0x01u << 5) | // LnaGain=G1 (max)
                                                       (cfg.lna_boost_rx ? 0x03u : 0x00u));
    if ((e = spi_.write_register(reg::kLna, lna)) != LoRaError::OK)
        return e;
    ...
    if ((e = spi_.write_register(reg::kInvertIq, inv_iq)) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kInvertIq2, inv_iq2)) != LoRaError::OK)
        return e;

    // Errata
    if ((e = apply_errata(cfg.bandwidth_hz, cfg.frequency_hz)) != LoRaError::OK)
        return e;
    ...
    if ((e = spi_.write_register(reg::kFifoTxBaseAddr, 0)) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kFifoRxBaseAddr, 128)) != LoRaError::OK)
        return e;

    // DIO mapping: DIO0=RxDone by default
    if ((e = spi_.write_register(reg::kDioMapping1, dio::kDio0RxDone)) != LoRaError::OK)
        return e;
    ...
    if ((e = spi_.write_register(reg::kIrqFlags, irq::kClearAll)) != LoRaError::OK)
        return e;
...
LoRaError SX127xDriver::set_lna_gain(std::uint8_t gain) noexcept {
    ...
    std::uint8_t mc3 = 0;
    if (spi_.read_register(reg::kModemConfig3, mc3) != LoRaError::OK)
        return LoRaError::SpiFailure;
    mc3 &= ~0x04u;
    const LoRaError e2 = spi_.write_register(reg::kModemConfig3, mc3);
    if (e2 != LoRaError::OK)
        return e2;
    const std::uint8_t lna =
        static_cast<std::uint8_t>((gain << 5) | (cfg_.lna_boost_rx ? 0x03u : 0x00u));
...
LoRaError SX127xDriver::start_continuous_wave() noexcept {
    ...
    LoRaError e;
    std::uint8_t mc2 = 0;
    if ((e = spi_.read_register(reg::kModemConfig2, mc2)) != LoRaError::OK)
        return e;
...
LoRaError SX127xDriver::start_transmit(const std::uint8_t* data, std::size_t len,
    ...
    if (len == 0u)
        return LoRaError::InvalidConfig;
    if (len > 255u)
        return LoRaError::TxBufferTooLarge;
    ...
    if ((e = spi_.write_register(reg::kFifoTxBaseAddr, 0)) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kFifoAddrPtr, 0)) != LoRaError::OK)
        return e;
    if ((e = spi_.burst_write(reg::kFifo, data, len)) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kPayloadLength, static_cast<std::uint8_t>(len))) !=
        LoRaError::OK)
        return e;

    // DIO0 = TxDone before entering TX mode
    if ((e = spi_.write_register(reg::kDioMapping1, dio::kDio0TxDone)) != LoRaError::OK)
        return e;

    if ((e = set_op_mode(opmode::kLoRaTx)) != LoRaError::OK)
        return e;
...
LoRaError SX127xDriver::start_receive(bool continuous) noexcept {
    ...
    if ((e = spi_.write_register(reg::kFifoRxBaseAddr, 128)) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kFifoAddrPtr, 128)) != LoRaError::OK)
        return e;
    if ((e = spi_.write_register(reg::kDioMapping1, dio::kDio0RxDone)) != LoRaError::OK)
        return e;
...
LoRaError SX127xDriver::read_packet(std::uint8_t* buf, std::size_t max_len,
    ...
    LoRaError e;
    if ((e = spi_.read_register(reg::kFifoRxCurrentAddr, rx_addr)) != LoRaError::OK)
        return e;
    if ((e = spi_.read_register(reg::kRxNbBytes, nb_bytes)) != LoRaError::OK)
        return e;
    if (nb_bytes == 0u)
        return LoRaError::OK;

    const std::size_t to_read = (nb_bytes <= max_len) ? nb_bytes : max_len;
    if ((e = spi_.write_register(reg::kFifoAddrPtr, rx_addr)) != LoRaError::OK)
        return e;
    if ((e = spi_.burst_read(reg::kFifo, buf, to_read)) != LoRaError::OK)
        return e;
    out_len = to_read;
    return LoRaError::OK;
}
...
std::int16_t SX127xDriver::current_rssi() const noexcept {
    if (!initialized_)
        return 0;
    std::uint8_t raw = 0;
    if (spi_.read_register(reg::kRssiValue, raw) != LoRaError::OK)
        return 0;
...
void SX127xDriver::process_events() noexcept {
    ...
    while (irq_tail_ != irq_head_ && iters < kIrqQueueSize) {
        ...
        std::uint8_t flags = 0;
        if (spi_.read_register(reg::kIrqFlags, flags) != LoRaError::OK)
            continue;
        if (flags == 0u)
            continue; // synthetic poll with nothing to do
    ...
    // Update backlog stat
    const std::uint8_t backlog =
        static_cast<std::uint8_t>((irq_head_ + kIrqQueueSize - irq_tail_) % kIrqQueueSize);
    if (backlog > stats_.max_irq_backlog)
        stats_.max_irq_backlog = backlog;
}
...
LoRaError SX127xDriver::run_rx_image_calibration() noexcept {
    ...
    LoRaError e;
    std::uint8_t v = 0;
    if ((e = spi_.read_register(reg::kImageCal, v)) != LoRaError::OK)
        return e;
    v |= 0x40u; // ImageCalStart bit
    if ((e = spi_.write_register(reg::kImageCal, v)) != LoRaError::OK)
        return e;

    // Wait for ImageCalRunning to clear (bit 5). Bounded poll, ~1 ms.
    for (int i = 0; i < 100; ++i) {
        if (spi_.read_register(reg::kImageCal, v) != LoRaError::OK)
            return LoRaError::SpiFailure;
        if ((v & 0x20u) == 0u)
            return LoRaError::OK;
#ifdef ARDUINO
